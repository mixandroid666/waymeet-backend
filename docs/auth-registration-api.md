# Ruammit — Registration API (Flutter integration guide)
...
> Scope: account creation only. **Login is not part of this API yet** — after
...
contact (email|phone) + password
        │  POST /auth/register
        ▼
   201 + OTP "sent"  ───────────────►  user enters 6-digit code
                                              │  POST /auth/verify-otp
                                              ▼
                                         200 verified  ──► back to LoginScreen
                                              ▲
...
|-------------|----------|
| Local (emulator → host) | Android emulator: `http://10.0.2.2:8080` · iOS sim/desktop/web: `http://localhost:8080` |
| Production | TBD |
...
```json
{
  ...
  "debug_code": "199317"            // DEV ONLY — omitted in production
}
```
...
> in production builds — it will be absent.
...
**Response `200 OK`** — same shape as register (`message`, `contact`,
...
| 409  | `already_verified`        | Contact already activated | "Account exists — please log in" |
...
- **`lib/screens/register_screen.dart`** — `_onRegister()` currently navigates
  ...
  and `_passwordController` — map them to the request fields.
- **`lib/screens/otp_screen.dart`** — `_verify()` currently just shows a snackbar.
  ...
  `OtpScreen` already receives `contact` and `isEmail` — derive `contact_type`
  from `isEmail`.
...
`_ContactType.email → "email"`, `_ContactType.phone → "phone"`. The backend
...
const String kApiBase = 'http://10.0.2.2:8080'; // Android emulator → host
...
try {
  ...
  // e.code == 'already_verified' → suggest logging in, etc.
  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message)));
}
```

> Add the `http` package to `ruammit-flutter/pubspec.yaml`
> (`flutter pub add http`) — it is not currently a dependency.
...
- **No display name collected at signup** — the `users.display_name` column is
